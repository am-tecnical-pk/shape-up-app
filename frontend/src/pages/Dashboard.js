...
// ==========================================
// 2. NEW AI WIDGET (With Sync Capability)
// ==========================================
const DailyBriefingWidget = ({ onBriefingLoaded }) => {
    const { data: briefing, isLoading, isSuccess } = useGetDailyBriefingQuery();

    // When AI finishes planning, refresh the dashboard stats
    useEffect(() => {
        if (isSuccess && briefing && onBriefingLoaded) {
            onBriefingLoaded();
        }
    }, [isSuccess, briefing, onBriefingLoaded]);
                ...
                <Card.Body className="d-flex align-items-center gap-4 p-4 position-relative z-1">
                    ...
                    <div>
                        ...
                        <p className="mb-0 fs-5 fw-light" style={{ lineHeight: "1.5", color: "#e2e8f0" }}>
                            {isLoading ? "Analyzing your recovery and setting today's targets..." : (briefing?.message || "Let's crush today's goals! Stay consistent.")}
                        </p>
                    </div>
                </Card.Body>
            </Card>
...
// ==========================================
const Dashboard = () => {
  // Added 'refetch' to sync with AI
  const { data: dailyLogs, isLoading: loadingLogs, refetch: refetchLogs } = useGetDailyLogsQuery();
  ...
  const GOALS = useMemo(() => {
    ...
    if (userInfo) {
        if(userInfo.macros) goals.calories = userInfo.macros.calories;
    ...
    // 3. AI OVERRIDE (Dynamic Goals from Today's Log)
    // If the AI Briefing set a specific target for today, we use that instead.
    const todayLog = dailyLogs?.find(l => l.date === todayDate);
    if (todayLog) {
        if (todayLog.targetCalories && todayLog.targetCalories > 0) {
            goals.calories = todayLog.targetCalories;
        }
        if (todayLog.targetSteps && todayLog.targetSteps > 0) {
            goals.steps = todayLog.targetSteps;
        }
    }

    return goals;
  }, [userInfo, dailyLogs, todayDate]);
  ...
  return (
    <div className="dashboard-wrapper position-relative overflow-hidden bg-light">
      ...
      <Container className="py-5 position-relative z-1">
        ...
        {/* --- 1. AI BRIEFING (Integrated with Dashboard Refresh) --- */}
        <DailyBriefingWidget onBriefingLoaded={refetchLogs} />
        ...
        {/* --- 3. STATS CARDS (Now Using AI Goals) --- */}
