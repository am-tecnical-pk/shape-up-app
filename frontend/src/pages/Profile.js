...
import { 
  ...
  FaChartLine, FaDownload, FaTrashAlt, FaExclamationTriangle
} from "react-icons/fa";

// ==========================================
// 1. UTILITY: ULTRA-AGGRESSIVE COMPRESSION
// ==========================================
const compressImage = (file) => {
    return new Promise((resolve, reject) => {
        ...
        reader.onload = (event) => {
            ...
            img.onload = () => {
                const maxWidth = 120; // Small avatar size
                const maxHeight = 120;
                const canvas = document.createElement('canvas');
                let width = img.width;
                let height = img.height;

                // Simple scaling to fit box
                if (width > height) {
                    if (width > maxWidth) {
                ...
                // Low quality JPEG (0.5) to keep string short
...
const Profile = () => {
  ...
  useEffect(() => {
    if (userInfo) {
        ...
        setAvatar(userInfo.avatar && userInfo.avatar.length > 20 ? userInfo.avatar : `https://ui-avatars.com/api/?name=${userInfo.name}&background=0D8ABC&color=fff`);
    }
  }, [userInfo]);
  ...
  const handleAvatarChange = async (e) => {
      ...
      if (file) {
          try {
              ...
              toast.info("Image prepared! Click 'Save Changes' to upload.");
          } catch (error) {
  ...
  const submitHandler = async (e) => {
    ...
    try {
        const payload = {
          ...
          name,
          email,
          avatar, // Compressed Base64 String
          age: Number(age),
        ...
        // Only send password fields if actually changing password
        ...
        if (err.status === 413) {
            toast.error("Image too large. Please use a smaller file.");
      ...
      <Container className="py-5" style={{maxWidth: '1200px'}}>
        ...
        <Modal show={showDeleteModal} onHide={() => setShowDeleteModal(false)} centered>
            ...
            <Modal.Footer className="justify-content-center border-0 pb-4">
                ...
                <Button variant="danger" className="fw-bold">Yes, Delete Account</Button>
            </Modal.Footer>
        </Modal>
