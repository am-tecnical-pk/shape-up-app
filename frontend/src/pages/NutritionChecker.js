...
// --- CHANGED: IMPORT FROM AI SLICE NOW ---
import { 
...
const NutritionChecker = () => {
  ...
  // --- CHANGED: USE NEW AI HOOKS ---
  const { data: activeDiet } = useGetMyDietQuery();
  const [generateDiet, { isLoading: generating }] = useGenerateDietPlanMutation();
  const [saveDiet, { isLoading: saving }] = useSaveDietPlanMutation();
  const [analyzeImage, { isLoading: analyzing }] = useAnalyzeFoodImageMutation(); 
