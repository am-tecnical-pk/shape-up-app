...
import { useSaveRoutineMutation, useGetMyRoutineQuery } from "../slices/routinesApiSlice"; 
...
const WorkoutPlan = () => {
  ...
  const [saveRoutine, { isLoading: saving }] = useSaveRoutineMutation();
          ...
          <Card className="glass-card mb-4 border-0 p-4">
            ...
            <Form onSubmit={handleGenerate}>
              ...
              <div className="d-flex gap-2">
                  ...
                  <Button type="submit" className={`py-3 rounded-pill fw-bold mt-2 shadow-sm ${existingRoutine ? 'w-75' : 'w-100'}`} disabled={loading}>
                    {loading ? <><Spinner size="sm" className="me-2"/>Analysing your profile...</> : "Generate My Plan ✨"}
                  </Button>
              </div>
            </Form>
          </Card>
