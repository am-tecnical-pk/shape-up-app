...
import Workout from "../models/workoutModel.js"; // 👈 IMPORTED WORKOUT MODEL
...
// 1. GENERATE DIET PLAN (Now with Workout Context)
const generateDietPlan = asyncHandler(async (req, res) => {
  ...
  // --- 1. GET WORKOUT CONTEXT ---
  // We need to know if the user trained recently to adjust macros
  const lastWorkout = await Workout.findOne({ user: req.user._id }).sort({ createdAt: -1 });
  let workoutContext = "User is resting today.";
  
  if (lastWorkout) {
      const workoutDate = new Date(lastWorkout.createdAt).toDateString();
      const today = new Date().toDateString();
      
      if (workoutDate === today) {
          workoutContext = `User TRAINED TODAY. Intensity: ${lastWorkout.rpe || 'Moderate'}/10. Focus: ${lastWorkout.exercises.length} exercises. Needs recovery fuel.`;
      } else {
          workoutContext = `Last workout was on ${workoutDate}. User is likely recovering or resting.`;
      }
  }
  ...
  const systemInstruction = `
    ...
    TRAINING CONTEXT: ${workoutContext}
    (If they trained today/recently, prioritize post-workout carbs and protein).
...
const analyzeFoodImage = asyncHandler(async (req, res) => {
  ...
  const systemInstruction = `
    ...
    OUTPUT JSON: { "name": "Food Name", "calories": 0, "protein": 0, "carbs": 0, "fat": 0, "serving": "1 bowl/plate", "confidence": "High" }
  `;
